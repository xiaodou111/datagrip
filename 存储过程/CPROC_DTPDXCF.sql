create procedure cproc_dtpdxcf(p_begindate in  date,
                                          p_enddate   in  date,
...
--320108  按公司分类桐乡
    ...
    v_days_hb := round(to_number(add_months(p_enddate,-1) - add_months(p_begindate,-1)) +1);
          ...
          from zb_period a
               join 
               (select  period,busno,dept,class,xsrw,mlerw FROM d_rw_rj_ztxsqk 
               UNION  all
                select  to_char(p_begindate,'yyyymm') as period,a.BUSNO,
                tb.classname AS dept,'12116' as class ,0 as xsrw,0 as mlerw from s_busi a
               join t_busno_class_set ts on a.busno=ts.busno and ts.classgroupno ='303'
               join t_busno_class_base tb on ts.classgroupno=tb.classgroupno and ts.classcode=tb.classcode  
               where not exists (select 1 from d_rw_rj_ztxsqk sqk where sqk.busno=a.busno and sqk.period=to_char(p_begindate,'yyyymm') 
               and sqk.class in ('12116','zxcy_yw') )
               and a.busno not like '89%' and a.busno not like '824%'
               union all   
               select  to_char(p_begindate,'yyyymm') as period,a.BUSNO,
                tb.classname AS dept,'zxcy_yw' as class ,0 as xsrw,0 as mlerw from s_busi a
               join t_busno_class_set ts on a.busno=ts.busno and ts.classgroupno ='303'
               join t_busno_class_base tb on ts.classgroupno=tb.classgroupno and ts.classcode=tb.classcode  
               where not exists (select 1 from d_rw_rj_ztxsqk sqk where sqk.busno=a.busno and sqk.period=to_char(p_begindate,'yyyymm') 
               and sqk.class in ('12116','zxcy_yw') )
               and a.busno not like '89%' and a.busno not like '824%'
                ) b on a.period=b.period and b.class in ('12116','zxcy_yw')
               --d_rw_rj_ztxsqk b on a.period=b.period and b.class in ('12116','zxcy_yw')
          ...
          select 1 as px1,case when z.classcode='12105' then 1 else 2 end as px2,
                 ...
                 nvl(a.netsum,0) as netsum,nvl(b.netsum_hb,0) as netsum_hb,nvl(c.netsum_tb,0) as netsum_tb,
          ...
          where --z.xszb<>0
                --and 
                exists(select 1 from t_busno_class_set tcb where tcb.classgroupno='305' and tcb.classcode in('30510','30511')and z.busno=tcb.busno )
