...
if  res.hd =1 then
 ...
 v_sql := v_sql||'CASE WHEN QTY>0 THEN  55 ELSE 56 end ,case when  QTY>0 THEN to_char(41801) else to_char(40101) END       FROM v_pf_rt03 where REC_DATE >= date'''||res.zdate||''' and ( REC_DATE > (select max(ORDER_DATE)  from d_accept_def where REGEXP_LIKE(VIEW_NAME,'''||p_vname||''',''i'') or 0='||v_cnt||' )' ;
end if ;

--拼接固化数据
v_sql:= 'create or replace view '||v_name||' as SELECT * FROM  ( '||v_sql  ||' ) and ORDER_DATE < trunc(sysdate)';
...
v_sql := v_sql ||' a.batch_id,a.actual_production_date,a.expiry_date,a.tax_rate,a.sup_price,a.sup_price_no_tax,a.total_amount,a.total_amount_no_tax,a.type_num_id,a.in_storage FROM   d_accept_def a where REGEXP_LIKE(VIEW_NAME,'''||p_vname||''',''i'') ' ;
--拼接导入数据
v_import:='union all select CORT_NUM_ID 公司编码, CORT_NAME 公司名称, ORDER_DATE 入库日期, SUPPLY_UNIT_NUM_ID 供应商, SUPPLY_NAME 供应商名称,
       ...
       SUP_PRICE_NO_TAX 采购订单单价不含税, TOTAL_AMOUNT 总金额含税, TOTAL_AMOUNT_NO_TAX 总金额不含税, TYPE_NUM_ID as 业务类型, IN_STORAGE
