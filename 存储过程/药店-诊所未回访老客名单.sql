...
BEGIN
    ...
    from (select execdate, customer, idcard, cbdid, cbd, a.busno, saleno, cbrylx,
    ...
    where
      --取参保地和药店所在地对应的数据
        CASE
            WHEN a.cbd IN ('台州市本级', '台州市椒江区', '台州市黄岩区', '台州市路桥区')
                --not like '%居民%'医保
                and CBRYLX not like '%居民%' THEN '医保市本级'
            when a.cbd IN ('台州市本级', '台州市椒江区') and CBRYLX like '%居民%' THEN '农保市本级'
            else a.cbd end
            = case
                  when REPLACE(tb2.classname, '台州市玉环市', '台州市玉环县') in
                       ('台州市本级', '台州市椒江区', '台州市黄岩区', '台州市路桥区')
                      and CBRYLX not like '%居民%' THEN '医保市本级'
                  when REPLACE(tb2.classname, '台州市玉环市', '台州市玉环县') IN ('台州市本级', '台州市椒江区') and
                       CBRYLX like '%居民%' THEN '农保市本级'
                  else REPLACE(tb2.classname, '台州市玉环市', '台州市玉环县') end
      and a.EXECDATE >= date'2024-01-01')
    WHERE rn = 1;
...
WHERE rn=1*/
    ...
    SELECT execdate, customer, idcard, cbdid, cbd, busno
    FROM (SELECT a.execdate, a.customer, a.idcard, a.cbdid, a.cbd, a.busno,
...
--d_zhyb_year_2022_notzs:有在药店消费,诊所未消费,诊所消费后去除
    ...
    select a.EXECDATE, a.CUSTOMER, a.IDCARD, a.CBDID, a.BUSNO, a.SALENO, a.TYPE, a.HFRY, bbb.EXECDATE
    from d_zhyb_year_2022_notzs a
    join d_zhyb_year_2023_zs bbb on a.IDCARD = bbb.IDCARD
    WHERE EXISTS(SELECT 1 FROM d_zhyb_year_2023_zs b WHERE a.idcard = b.idcard
      AND b.execdate between TRUNC(SYSDATE) - 1 AND TRUNC(SYSDATE));

    delete from d_zhyb_year_2022_notzs a
    where EXISTS(SELECT 1 FROM d_zhyb_year_2023_zs b  WHERE a.idcard = b.idcard
      AND b.execdate between TRUNC(SYSDATE) - 1 AND TRUNC(SYSDATE));
 --d_zhyb_year_2022_notyd  药店核心会员,药店消费后去除

    INSERT INTO d_notyd_hfjl (EXECDATE, CUSTOMER, IDCARD, CBDID, BUSNO, SALENO, TYPE, HFRY, accdate)
    select a.EXECDATE, a.CUSTOMER, a.IDCARD, a.CBDID, a.BUSNO, a.SALENO, a.TYPE, a.HFRY, bbb.EXECDATE
    from d_zhyb_year_2022_notyd a
    join d_zhyb_year_2023_yd bbb on a.IDCARD = bbb.IDCARD
    WHERE EXISTS(SELECT 1 FROM d_zhyb_year_2023_yd b WHERE a.idcard = b.idcard
      AND b.execdate between TRUNC(SYSDATE) - 1 AND TRUNC(SYSDATE));

    delete from  d_zhyb_year_2023_yd a
    where EXISTS(SELECT 1 FROM d_zhyb_year_2023_yd b  WHERE a.idcard = b.idcard
      AND b.execdate between TRUNC(SYSDATE) - 1 AND TRUNC(SYSDATE));
...
--插入前一天数据中 没在诊所消费的数据(在药店消费的数据)  并把表中原先有身份证的删除
    ...
    SELECT EXECDATE, CUSTOMER, IDCARD, CBDID, CBD, BUSNO, SALENO, CBRYLX, '诊所核心会员'
...
--补参保单位
    ...
    FROM d_zhyb_hz_cyb
    where ERP销售单号 is not null
      and EXT_CHAR04 is not null
      and 销售日期 between TRUNC(SYSDATE) - 2 AND TRUNC(SYSDATE)
    group by ERP销售单号, EXT_CHAR04) b
    ON (a.SALENO = b.ERP销售单号)
    WHEN MATCHED THEN
        UPDATE
        SET a.CBDW = b.EXT_CHAR04;
