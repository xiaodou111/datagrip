create or replace PROCEDURE proc_cw_pro_check(
                                          p_busno IN number,
...
left join t_stall st on d.stallno=st.STALLNO
 where sb.ZMDZ1 = v_zmdz1  and d.WAREQTY <> 0
...
res as (
--   select zmdz1, BUSNO, WAREID, 可用数量, 商品数量, 待出库数量, 待处理数量, SALEPRICE, je, MAKENO, STALLNAME, sumsl, rn
 select
      COALESCE(d_sl.busno, d_je.busno, d_zx.busno) as busno,
     COALESCE(d_sl.WAREID, d_je.WAREID, d_zx.WAREID) AS WAREID,
     COALESCE(d_sl.商品数量, d_je.商品数量, d_zx.商品数量) as 商品数量,
     COALESCE(d_sl.待出库数量, d_je.待出库数量, d_zx.待出库数量) as 待出库数量,
     COALESCE(d_sl.待处理数量, d_je.待处理数量, d_zx.待处理数量) as 待处理数量,
     COALESCE(d_sl.可用数量, d_je.可用数量, d_zx.可用数量) as 可用数量,
     COALESCE(d_sl.SALEPRICE, d_je.SALEPRICE, d_zx.SALEPRICE) as SALEPRICE,
     COALESCE(d_sl.MAKENO, d_je.MAKENO, d_zx.MAKENO) as MAKENO,
     COALESCE(d_sl.STALLNAME, d_je.STALLNAME, d_zx.STALLNAME) as STALLNAME,
        d_sl.rn as 库存排名
...
select r.busno,s.ORGNAME, r.WAREID,w.WARENAME,w.WAREUNIT,w.WARESPEC,f.FACTORYNAME
     ,r.可用数量, r.商品数量,r.待出库数量,r.待处理数量, r.SALEPRICE,r.商品数量*r.SALEPRICE as 金额,MAKENO,STALLNAME, r.库存排名, r.金额排行, r.滞销排行
from res r
...
union all
select  a1.busno,s.ORGNAME, a1.WAREID,w.WARENAME,w.WAREUNIT,w.WARESPEC, f.FACTORYNAME,a1.可用数量,a1.商品数量,a1.待出库数量,a1.待处理数量, a1.SALEPRICE, a1.je,MAKENO,STALLNAME, null,null,null
from a1
left join s_busi s on a1.busno =s.BUSNO
left join t_ware_base w on a1.WAREID=w.WAREID
left join t_factory f on w.FACTORYID=f.FACTORYID
union all
select  a2.busno,s.ORGNAME, a2.WAREID,w.WARENAME,w.WAREUNIT,w.WARESPEC, f.FACTORYNAME,a2.可用数量,a2.商品数量,a2.待出库数量,a2.待处理数量, a2.SALEPRICE, a2.je,MAKENO,STALLNAME, null,null,null
from a2
