create or replace PROCEDURE proc_cw_pro_check(p_busno IN number,
...
SELECT ZMDZ1,busno,WAREID, 总数量,待出库数量总计,总不合格数量,总待验数量,可用数量,sumsl,SALEPRICE,sumsl*SALEPRICE as je, DENSE_RANK() OVER (ORDER BY sumsl DESC) AS rn,lx,
        ...
        sum(d.SUMDEFECTQTY) as 总不合格数量,sum(SUMTESTQTY) as 总待验数量,sum(SUMQTY-SUMAWAITQTY-SUMDEFECTQTY-SUMTESTQTY) as 可用数量,sum(sum(d.SUMQTY))over ( partition by s.ZMDZ1,d.WAREID) as sumsl,tws.SALEPRICE,
                     ...
                     from T_STORE_h d
                              join s_busi s on d.BUSNO = s.BUSNO
                      left join t_ware_saleprice tws on tws.compid=d.COMPID and tws.salegroupid NOT LIKE '91%' and tws.salegroupid='1000001' and d.WAREID=tws.WAREID
                       join t_ware_class_base tc on tc.CLASSCODE in ('01120301','01120306','01120307') and TC.compid = d.COMPID and tc.WAREID = d.WAREID and TC.classgroupno = '01'
                     where d.COMPID = d.COMPID and s.ZMDZ1 = v_zmdz1
...
SELECT ZMDZ1,busno,WAREID, 总数量,待出库数量总计,总不合格数量,总待验数量,可用数量,sumsl,SALEPRICE,sumsl*SALEPRICE as je, DENSE_RANK() OVER (ORDER BY sumsl DESC) AS rn,
        ...
        sum(d.SUMDEFECTQTY) as 总不合格数量,sum(SUMTESTQTY) as 总待验数量,sum(SUMQTY-SUMAWAITQTY-SUMDEFECTQTY-SUMTESTQTY) as 可用数量,sum(sum(d.SUMQTY))over ( partition by s.ZMDZ1,d.WAREID) as sumsl,tws.SALEPRICE
                     ...
                     from T_STORE_h d
                              join s_busi s on d.BUSNO = s.BUSNO
                      left join t_ware_saleprice tws on tws.compid=d.COMPID and tws.salegroupid NOT LIKE '91%' and tws.salegroupid='1000001' and d.WAREID=tws.WAREID
                       join t_ware_class_base tc on substr(TC.classcode, 1, 6) ='011201' and TC.compid = d.COMPID and tc.WAREID = d.WAREID and TC.classgroupno = '01'
                     where d.COMPID = d.COMPID and s.ZMDZ1 = v_zmdz1
                     group by s.ZMDZ1,s.BUSNO,d.WAREID,tws.SALEPRICE
                     ))
 where rn<=3 ),
...
select r.busno,s.ORGNAME, r.WAREID,w.WARENAME,w.WAREUNIT,w.WARESPEC,f.FACTORYNAME
     , r.总数量,r.待出库数量总计,r.总不合格数量,r.总待验数量,r.可用数量, r.SALEPRICE,r.总数量*r.SALEPRICE as 金额,
       r.库存排名, r.金额排行, r.滞销排行
from res r
