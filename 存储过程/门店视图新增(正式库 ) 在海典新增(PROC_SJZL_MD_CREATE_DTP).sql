create procedure proc_sjzl_md_create_dtp(p_name in varchar2,
                                                p_busno  in    varchar2,
...
where d.wareid in (select wareid from '||p_waretable||')  and to_char(h.execdate,''yyyy-mm-dd'') >=''2023-01-01''';
...
AND a.busno in ('|| p_busno ||')  and a.accdate>=date''2023-01-01''  and  a.accdate< trunc(sysdate)';
