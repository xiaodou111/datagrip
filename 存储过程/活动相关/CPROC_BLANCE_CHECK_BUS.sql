...
begin
 ...
 /* 2021-05-07 新增审核过的机构无法刷新 */
select count(*) into v_cnt from d_balance_check where busno=p_busno and accdate =p_check_month and check_status =1;
if  v_cnt > 0 then
    RAISE_APPLICATION_ERROR(-20001, '本月已经审核无法刷新数据!', TRUE);
    RETURN;
end if;
...
20220926
考核类别加入判断： 只针对  西药 保健 器械 日化
A考核类别大类9011
B考核类别大类9012+9017+9018
C考核类别大类9013
D考核类别大类9014
E考核类别大类9015
其他考核类别大类9018+9020
...
delete from d_balance_check where busno=p_busno and accdate =p_check_month;

--先批量生成当月数据模板
/*select count(*) into v_cnt from d_check_computation a where not exists(select 1 from d_balance_check b where a.check_month=b.accdate and a.busno=b.busno) and a.check_month =p_check_month and a.busno=p_busno;
...
--刷新赔付比例
  ...
  USING (  select busno,zxcybl as pfbl,'12'||b.classcode as classgroupno  from d_check_compensation a,
           (SELECT REGEXP_SUBSTR('A,B,C,D,E,Q', '[^,]+', 1, rownum) as classcode FROM DUAL
           CONNECT BY ROWNUM <=LENGTH('A,B,C,D,E,Q') - LENGTH(REPLACE('A,B,C,D,E,Q', ',', NULL)) + 1) b where busno=p_busno
            union all
           ...
           CONNECT BY ROWNUM <=LENGTH('A,B,C,D,E,Q') - LENGTH(REPLACE('A,B,C,D,E,Q', ',', NULL)) + 1) b where busno=p_busno
            union all
            select busno,zyypbl as pfbl,'04' as classgroupno  from d_check_compensation where busno=p_busno
            union all
            select busno,srgxbl as pfbl,'05' as classgroupno  from d_check_compensation where busno=p_busno
            union all
            select busno,cxbzbl as pfbl,'06' as classgroupno  from d_check_compensation where busno=p_busno) T2
...
where a.status=1 and a.billcode='ABN' and a.busno=p_busno
...
where a.status=1 and a.busno=p_busno
GROUP  BY a.busno;
...
--刷新盘点结算表  损益 ：DTP和冷藏商品按照差异赔付，其他的按欠款赔付
  ...
  update d_balance_check set jsprice=0 where accdate=p_check_month and classgroupno not in('02','03') and abnprice>0 and busno=p_busno ;
  --物资恒为零
  update d_balance_check set jsprice=0 where accdate=p_check_month and classgroupno ='08'and busno=p_busno ;
...
--结算金额
 ...
 --固定资产盘点结算结算金额
 MERGE INTO d_balance_check T1
  USING (select MONTH, 8||substr(BUSNO,-4) as BUSNO,'16' as classgroupno, JE from d_gdzc_pd) T2
  ON ( T1.busno=T2.busno and T1.classgroupno=T2.classgroupno and T1.ACCDATE=p_check_month )
  WHEN MATCHED THEN
