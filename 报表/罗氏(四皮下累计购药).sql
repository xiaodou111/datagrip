...
with first as (select a.SALENO, a.ACCDATE, d.WAREQTY,
               ...
               where a.ACCDATE >= date'2022-01-01' and d.WAREID in (10601875) ---10502445,
...
--        该方案曲妥珠单抗是否为赫赛汀, 疾病分期, 是否早期新辅助治疗,
       ...
       case
           when files.新皮下方案 = '双靶(曲妥珠单抗HSC+帕妥珠单抗)' and k转皮下后帕妥珠单抗支数 < q本店累计购药次数 then '重新核查盒数'
       ...
       ac本店第一次购药时间, rn2
from after10601875 aa
left join d_luoshi_px_hf hf on aa.IDCARDNO=hf.idcard and hf.busno=aa.BUSNO
