with base as (select a.SALENO, a.ACCDATE, d.WAREQTY,
...
-- null as l二二年1月以前累计购药盒数,
                   ...
                   (MAX(a.ACCDATE) OVER (PARTITION BY h.IDCARDNO ) - MIN(a.ACCDATE) OVER (PARTITION BY h.IDCARDNO ) +
                    21) / 21 as N理论购药支数,
...
where a.ACCDATE >= date'2022-01-01'
  and d.WAREID in (10600308, 10502445) ---10502445,
...
left join d_luoshi_qcsj qc on base.IDCARDNO=qc.IDCARDNO
where rn = 1;
