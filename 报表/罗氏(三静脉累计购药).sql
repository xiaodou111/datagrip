...
sum(d.WAREQTY)
    over ( partition by h.IDCARDNO,a.busno,d.WAREID order by a.ACCDATE ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) m二二年1月以来本店累计购药支数,
Max(a.ACCDATE) OVER (PARTITION BY h.IDCARDNO,a.busno,d.WAREID ) AS r本店最近一次购药时间,
LAG(a.ACCDATE, 1) OVER (PARTITION BY h.IDCARDNO,a.busno,d.WAREID ORDER BY a.ACCDATE ) AS s本店前一次购药时间,
