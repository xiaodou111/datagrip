...
create table d_luoshi_jmsf_1 as select busno, ORGNAME, 药店所在省份, 药店所在城市, IDCARDNO, USERNAME, 用药方案,
...
delete from  d_luoshi_jmsf_1;
select * from d_luoshi_jm_hf;

--报表
select a.BUSNO, ORGNAME, "药店所在省份", "药店所在城市", a.IDCARDNO, USERNAME, fi.原用药方案 as 用药方案, fi."该方案曲妥珠单抗是否为赫赛汀",
...
a.SALENO, a.ACCDATE, d.WAREQTY,
h.busno, s.ORGNAME, tb.CLASSNAME as 药店所在省份, tb1.CLASSNAME as 药店所在城市, d.WAREID, w.WARENAME, h.IDCARDNO,
USERNAME, null as 用药方案, null as 该方案曲妥珠单抗是否为赫赛汀, null as 疾病分期, null as 是否早期新辅助治疗,
null as k患者本店总购药支数,
null as l二二年1月以前累计购药盒数,
...
--        WAREID, WARENAME,
       ...
       k患者本店总购药支数,
       nvl(l二二年1月以前累计购药盒数,0) as l二二年1月以前累计购药盒数,
       m二二年1月以来本店累计购药支数,
       N理论购药支数,
       case when N理论购药支数 - m二二年1月以来本店累计购药支数 >= 1 then '有非本店购买可能' else '皆在本店购买' end as o实际药房购药期间盒数偏差分析,
       case when m二二年1月以来本店累计购药支数 - q本店累计购药次数 < 0 then '重新核查盒数' else '0' end as p本店购买盒数核查,
