with a1 as (select a.SALENO, a.ACCDATE, d.WAREQTY,
                   h.busno, s.ORGNAME, tb.CLASSNAME as 药店所在省份, tb1.CLASSNAME as 药店所在城市, d.WAREID,
                   w.WARENAME, h.IDCARDNO,
                   h.USERNAME,
-- null as 用药方案, null as 该方案曲妥珠单抗是否为赫赛汀, null as 疾病分期, null as 是否早期新辅助治疗,
-- null as k患者本店总购药支数,
-- null as l二二年1月以前累计购药盒数,
                   sum(d.WAREQTY)
                       over ( partition by h.IDCARDNO order by a.ACCDATE ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) m二二年1月以来本店累计购药支数,
                   Max(a.ACCDATE) OVER (PARTITION BY h.IDCARDNO ) AS r本店最近一次购药时间,
                   LAG(a.ACCDATE, 1) OVER (PARTITION BY h.IDCARDNO ORDER BY a.ACCDATE ) AS s本店前一次购药时间,
                   min(a.ACCDATE) OVER (PARTITION BY h.IDCARDNO ) AS ac本店第一次购药时间,
                   (Max(a.ACCDATE) OVER (PARTITION BY h.IDCARDNO ) -
                    min(a.ACCDATE) OVER (PARTITION BY h.IDCARDNO ) +
                    21) / 21 as N理论购药支数,
...
--                 and h.IDCARDNO = '330106196808190140'
    ...
    sale_22 as (
     ...
     select h.IDCARDNO,
                         sum(d.WAREQTY)
                             over ( partition by h.IDCARDNO order by a.ACCDATE ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)
                             as 二二年以前累计购药盒数,
                        row_number() over (partition by h.IDCARDNO order by a.ACCDATE desc) rn22
                  from t_remote_prescription_h h
                           join t_sale_h a on substr(a.notes, 0,
                                                     decode(instr(a.notes, ' '), 0, length(a.notes) + 1,
...
-- select * from a1 where rn=1 and a1.IDCARDNO='511023197804048681';
select
    a1.busno,
...
--        WAREID, WARENAME,
       ...
       files.疾病分期,
       files.是否早期新辅助治疗,
       nvl(sale_22.二二年以前累计购药盒数,0)+nvl(m二二年1月以来本店累计购药支数,0) as  k患者本店总购药支数,
       nvl(sale_22.二二年以前累计购药盒数,0) as l二二年1月以前累计购药盒数,
       m二二年1月以来本店累计购药支数,
       N理论购药支数,
       ...
       case when nvl(sale_22.二二年以前累计购药盒数,0)+nvl(m二二年1月以来本店累计购药支数,0) >= 19 then 'Y' else 'N' end as v推测是否已完成疗程,
...
--        hf.sfday as 随访时间, hf.sfresult as 随访反馈, hf.notes as 随访备注,
       ac本店第一次购药时间,rn
from  a1
left join sale_22 on a1.IDCARDNO=sale_22.IDCARDNO
