...
--select * from d_bl_saleno
 FOR rec_sale IN (SELECT a.saleno,b.busno, b.md_busno, trunc(a.accdate) AS accdate, MAX(a.compid) AS compid
                     ...
                     WHERE /* a.accdate > trunc(to_date('2021-03-19', 'yyyy-mm-dd') - 1) AND
                           ...
                           a.saleno in (
                           select saleno from d_saleno_temp where status=0)
...
BEGIN
  ...
  -- DBMS_OUTPUT.PUT_LINE('hd_msg_out插入了 ' || i || ' 行数据'||'msg_id:'||v_msg_id);
   ...
   insert into d_bl_saleno(saleno, msg_id,status) values(rec_sale.saleno,v_msg_id,0) ;
...
--select * from d_sale_errmsg where busno=81161 and accdate=date'2023-10-23';
--select saleno from d_saleno_temp a where not exists (select saleno from d_bl_saleno b where a.saleno=b.saleno) ; 
--select * from  d_bl_saleno where saleno in (select saleno from d_saleno_temp)
--alter table d_bl_saleno add msg_id varchar2(40); 
