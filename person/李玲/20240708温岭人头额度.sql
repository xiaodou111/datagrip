--todo 温岭人头额度(参保地为温岭的药店诊所算人头的时候合并),额度这里不准,用下面的
select 险种, qy,
--        sum(zed1) as 总额度, sum(zed) as 国谈总额度,   --额度这里不准,用下面的
       count(身份证号) as 人头
from (select a.ERP销售号, a.创建时间, a.机构编码, a.身份证号, a.险种, a.参保地,
             ...
             ROW_NUMBER() OVER (PARTITION BY
                 CASE
                     WHEN a.险种 = '职工基本医疗保险' and
                          TB2.CLASSCODE IN ('324331001', '324331002', '324331003', '324331004') THEN '324331001'
                     WHEN a.险种 = '城乡居民基本医疗保险' and TB2.CLASSCODE IN ('324331001', '324331002')
                         THEN '324331002'
                     ELSE TB2.CLASSCODE
                     END,
                 case when a.参保地 = '温岭市' then '30510' else TB22.CLASSCODE end ,
                 A.身份证号,A.险种 ORDER BY A.创建时间 ) rn
      from d_zjys_wl2023xse a
...
--期初额度算法
--todo 额度算法,下面是按区域汇总的
with base as (select d_zjys_wl2023xse.erp销售号, sfzs, 事业部, d_zjys_wl2023xse.身份证号, 创建时间,
                     险种, 机构编码,
              ...
              where trunc(创建时间) BETWEEN date'2023-01-01' AND date'2023-12-31'
--                and d_zjys_wl2023xse.机构编码 in ('85027','85034','85036','85037','85039','85040','85041','85042','85064','85067','85069','85074','85083','85084','89074','89075')
     ...
     base2 as (select 2024 as 年度, trunc(创建时间) as 会计日, 机构编码 as 普通门店编码, sfzs as 门店类型,
                      ...
                      case
                          when ROW_NUMBER() over (partition by 身份证号,险种,sfzs,就医地,
                              ...
                              ROW_NUMBER() over (partition by 身份证号,险种,sfzs,就医地,
                                  case
                                      when 险种 = '职工基本医疗保险' and 参保地 in ('市本级', '黄岩区', '路桥区')
                                          then '市本级'
                                      when 险种 = '城乡居民基本医疗保险' and 参保地 in ('市本级') then '市本级'
                                      else 参保地 end
...
select 2023 as 年度, 会计日, 普通门店编码, 门店类型, 就医地, 参保地, 险种类型, 医保类型,
       ...
       sum(ord2) as 人头,
       case
           when sum(ord2) = 0 then 0
           else
               (sum(总额度) - sum(国谈额度)) / sum(ord2) end as 人头基金,
       sum(总费用), sum(总额度),
       sum(医疗费用自费总额), sum(医疗费用自理总额), sum(国谈额度), sum(基本医疗统筹支付), sum(当年账户支付),
       sum(公务员补助统筹支付), sum(大病保险支付),
       sum(历年账户支付), sum(现金金额)
from base2
where 会计日 >= DATE'2023-01-01' and trunc(会计日) <= date'2023-12-31' and 就医地 ='温岭市'
group by 会计日, 普通门店编码, 门店类型, 就医地, 参保地, 险种类型, 医保类型;

update dm_yb_md_head_sum_qc
set RECEIPT_DATE=TO_CHAR(
...
--                and d_zjys_wl2023xse.机构编码 in ('85027','85034','85036','85037','85039','85040','85041','85042','85064','85067','85069','85074','85083','85084','89074','89075')
     ...
     base2 as (select 2024 as 年度, trunc(创建时间) as 会计日, 机构编码 as 普通门店编码, sfzs as 门店类型,
                      ...
                      基本医疗统筹支付, 当年账户支付, 公务员补助统筹支付, 大病金额 AS 大病保险支付, 历年账户支付, 现金金额
               from base)
select 2023 as 年度, 参保地, 险种类型, 医保类型,
       sum(case when ord > 1 then 0 else ord end) as 人次,
       sum(ord2) as 人头,
       case
           when sum(ord2) = 0 then 0
