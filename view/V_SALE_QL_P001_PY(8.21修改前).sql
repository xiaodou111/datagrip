...
SELECT
  ...
  from V_ACCEPT_QL_P001_PY a
  where matnr in (
 select WAREID FROM D_QL_WARE_PY
  )
 and  LGORT IN('P005','P006','P007','P888','P889') AND  djlx LIKE '%入库%' and WERKS='D001'
--     and 1=0
 union ALL
 SELECT
      a.zdate,a.WERKS,'台州瑞人堂药业有限公司' as zname1,
...
SELECT
 ...
 and LGORT not IN ('P005','P006','P007','P888','P889')   AND ZODERtype in ('2','4','5')  AND LIFNR in ('110673','110388','110030','110473')
 and zdate>=date'2024-04-01'
--   and 1=0
 union all
SELECT
      a.zdate,a.WERKS,decode(a.WERKS,'D001','台州瑞人堂药业有限公司','D010','浙江瑞人堂医药连锁有限公司') as zname1,
...
-- --D010出入库
-- SELECT
--       a.zdate,a.WERKS,decode(a.WERKS,'D001','台州瑞人堂药业有限公司','D010','浙江瑞人堂药业有限公司') as zname1,
--        LGORT,case when ZODERtype in('4','5') then '盘亏' else '批发出库单' end as djlx,a.lgobe,MATNR,
--        MAKTX,ZGUIG,zscqymc,
--      case when ZODERtype in('4','5') then '盘亏' else  name1 end as orgname,
--       0 AS rksl,a.menge AS cksl,a.zgysph AS ph,a.mseh6 AS dw,
...
--   select WAREID FROM D_QL_WARE_PY
--   )
--  and LGORT  IN ('P001') AND WERKS in ('D010')  AND ZODERtype in ('2','4','5')  AND LIFNR in ('110673','110388','110030','110473')
--  and zdate>=date'2024-04-01'
--   and 1=0
--  union all
-- SELECT
...
--   select WAREID FROM D_QL_WARE_PY
--   )
--  and LGORT IN ('P001') AND WERKS in ('D001')  AND ZODERtype in ('2','4','5') AND LIFNR in ('110673','110388','110030','110473')
--  and zdate>=date'2024-04-01'
--   and 1=0
 ...
 and a.WERKS in ('D001') and a.LGORT  IN ('P001','P021','P018','P025')  and i.lgort in('P005','P006','P007','P888','P889')  AND a.ZODERtype=3  AND a.LIFNR in ('110673','110388','110030','110473')
 and a.zdate>=date'2024-04-01'
   --and 1=0
 union all
  --P888、P006移仓给P001
 ...
 and i.WERKS in ('D001') and a.LGORT  IN ('P001','P021','P018','P025') AND i.lgort in('P005','P006','P007','P888','P889')  AND a.ZODERtype=3  AND a.LIFNR in ('110673','110388','110030','110473')
 and a.zdate>=date'2024-04-01'
/
