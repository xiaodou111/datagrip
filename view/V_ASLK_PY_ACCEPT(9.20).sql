...
FROM (select *
      ...
      union all
      SELECT a.cort_num_id,
             a.cort_name,
             a.order_date,
             a.supply_unit_num_id,
             a.supply_name,
             a.item_num_id,
             a.item_name,
             a.qty,
             ...
             a.approval_no,
             a.batch_id,
             a.actual_production_date,
             a.expiry_date,
             a.tax_rate,
             a.sup_price,
             a.sup_price_no_tax,
             a.total_amount,
             a.total_amount_no_tax,
             a.type_num_id,
             a.in_storage,
             bill_type
      FROM d_accept_def a
      ...
      union all
      select CORT_NUM_ID 公司编码,
             CORT_NAME 公司名称,
             ORDER_DATE 入库日期,
             SUPPLY_UNIT_NUM_ID 供应商,
             SUPPLY_NAME 供应商名称,
             ITEM_NUM_ID 商品编码,
             ITEM_NAME 商品名称,
             QTY 数量,
             ...
             APPROVAL_NO 批准文号,
             BATCH_ID 批号,
             ACTUAL_PRODUCTION_DATE 生产日期,
             EXPIRY_DATE 有效期,
             TAX_RATE 税率进项,
             SUP_PRICE 采购订单单价含税,
             SUP_PRICE_NO_TAX 采购订单单价不含税,
             TOTAL_AMOUNT 总金额含税,
             TOTAL_AMOUNT_NO_TAX 总金额不含税,
             TYPE_NUM_ID as 业务类型,
             IN_STORAGE,
             bill_type
