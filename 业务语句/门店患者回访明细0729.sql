SELECT to_char(a.accdate,'yyyymm') as amonth,a.accdate ,a.saleno,a.wareid,d.warename,d.warespec,e.factoryname,a.saler,su.username as salername,a.wareqty,a.netprice,a.netamt,a.invalidate,th.membercardno,tr.cfno,
...
nvl(tr.ext_str7,f.ext_str7) as ext_str7,nvl(tr.ext_str7,f.ext_str7)+b.hf_day as next_sfday,
...
nvl(tr.IDCARDNO,f.idcard) as IDCARDNO,a.busno,c.orgname,oo.rn,
b.hf_day,
case when hf_zt=1 then a.accdate+b.hf_day else  a.accdate+(b.hf_day+3)*a.wareqty-3 end as next_day,a.makeno,a.rowno,
f.lastmodify,f.lasttime,case when  pay.saleno is null then 0 else 1 end as ifyb
...
left join ( SELECT a.cfno,a.createtime,trim(a.username) as kk,row_number() over(partition by trim(a.username) order by a.createtime) as rn FROM t_remote_prescription_h  a
 ...
 WHERE a.accdate between trunc(add_months(sysdate,-7)) and trunc(sysdate)  and a.accdate>=date'2022-09-01' 
